...
- `Settings -> Cloud Providers` includes:
  ...
  - per-provider accordions with refresh/remove actions, API-key update forms, optional provider links (`Create account`, `Create token`, `Data security`), available-model toggles, and inline enabled-model settings for cloud thinking prompt switches, context size, output tokens, temperature, Top P, and Top K where supported
  ...
  - selected remote models preserve any tool/function support detected from provider metadata, and each selected model also exposes explicit `Enable built-in tools` and `Enable thinking control` settings so users can opt into prompt-based tool calling and model-specific thinking system-prompt switches when they know the model supports them
